...
{
    Logger::instance().logUserAction("DataGeneratorWidget: " + actionName);
}
...
{
    logAction("Остановить генерацию");
    SimulationView* view = SimulationManager::instance().simulationView();
    if (view) {
        view->stopSimulation();
...
{
    logAction("Перезапустить генерацию");
    SimulationView* view = SimulationManager::instance().simulationView();
    if (view) {
...
{
    logAction(QString("Имитация ДТП: %1").arg(checked ? "ВКЛ" : "ВЫКЛ"));
...
{
    logAction(QString("Вероятность ДТП: %1").arg(text));
    auto& manager = SimulationManager::instance();
    if (manager.accidentManager()) {
...
{
    logAction(QString("Неправильная парковка: %1").arg(checked ? "ВКЛ" : "ВЫКЛ"));
    auto& manager = SimulationManager::instance();
...
{
    logAction(QString("Вероятность неправильной парковки: %1").arg(text));
    auto& manager = SimulationManager::instance();
    if (manager.simulationView()) {
        // Парсим текст вероятности (например, "10%" -> 0.1)
        double probability = 0.0;
